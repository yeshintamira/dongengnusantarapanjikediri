...
def add_styles():
    st.markdown(
        ...
        /* Gaya untuk modal cerita lengkap */
        .modal-content {
...
def load_lda_model():
    ...
    try:
        ...
        if not all(key in model_data for key in ['lda_model', 'dictionary', 'preprocessed_test', 'data_test', 'file_paths_test']):
            st.error("Struktur model LDA tidak lengkap.")
            return None
        ...
        st.error("File model LDA tidak ditemukan.")
        return None
    except Exception as e:
...
def load_lsi_model():
    ...
    try:
        ...
        if not all(key in model_data for key in required_keys):
            st.error("Struktur model LSI tidak lengkap.")
            return None
            
        return model_data
        
    except FileNotFoundError:
        st.error("File model LSI tidak ditemukan.")
        return None
    except Exception as e:
...
def preprocess_text(text, model_data):
    """Pra-pemrosesan teks untuk model LDA"""
...
def preprocess_user_input(text, model_data):
    """Pra-pemrosesan input pengguna untuk model LDA"""
...
def preprocess_lsi_query(text):
    """Pra-pemrosesan teks untuk model LSI"""
    text = text.lower()
    text = re.sub(r'[^a-z\s]', ' ', text)
    tokens = word_tokenize(text)
...
def extract_search_keywords(user_input):
    """Mengekstrak kata kunci dari input pengguna"""
...
def clean_story_content(content):
    """Membersihkan konten cerita"""
    content = re.sub(r'[^\w\s.,!?\'"-]', ' ', content)
    content = re.sub(r'\s+', ' ', content)
    return content.strip()

def format_full_story(content, keywords=None):
    """Memformat cerita lengkap untuk ditampilkan"""
...
def cosine_similarity_manual(vec1, vec2):
    """Menghitung kesamaan kosinus manual"""
...
def keyword_similarity(query_tokens, doc_tokens):
    """Menghitung kesamaan kata kunci"""
...
def recommend_with_lda(user_input, model_data):
    """Rekomendasi dokumen menggunakan model LDA"""
    if model_data is None:
        return []
    ...
    for idx, doc_tokens in enumerate(model_data['preprocessed_test']):
        ...
        if combined_score >= min_similarity:
            ...
            title = re.sub(r'[^a-zA-Z0-9\s]', ' ', file_name)
...
def recommend_with_lsi(user_input, model_data):
    """Rekomendasi dokumen menggunakan model LSI"""
    if model_data is None:
        return []
